...
import { type DataTableColumnDef, DataTableKeys } from "../type";
...
export type DataTableHeadCellProps<T> = Omit<
...
export const DataTableHeadCell = <T,>(props: DataTableHeadCellProps<T>) => {
  ...
  const disableResize = useMemo(
    () => lastCol || col.key === DataTableKeys.SELECT,
    [col, lastCol]
  );
  ...
  useEffect(() => {
    if (resizable && lastCol && !width) {
      console.error(
        "If Resizable is present, the last column must be passed a width parameter."
      );
    }
  }, [lastCol, width]);
  ...
  const sortCpn = useMemo(() => {
    ...
    const SortIcon =
      sortValue?.orderDirection === "desc" ? ArrowDownward : ArrowUpward;
